<script lang="ts">
  ...
  function clearSelectedFiles() {
    files = []
    const input = document.getElementById('lost-found-files') as HTMLInputElement | null
...
<div class="min-h-[calc(100vh-4rem)] px-4 py-5 sm:px-6 lg:px-8">
  <div class="mx-auto max-w-3xl space-y-4">
    <section class="rounded-3xl border border-cyan-100 bg-white/80 p-4 shadow-sm backdrop-blur-sm sm:p-5">
      ...
      <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-900">Report Lost/Found Item</h1>
      <p class="mt-1 text-sm text-slate-600">Use concise details. Better details mean faster item matching.</p>
    </section>
    ...
    {:else}
      <section class="space-y-3.5 rounded-3xl border border-cyan-100 bg-white/85 p-4 shadow-sm backdrop-blur-sm">
        {#if error}
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="text-xs font-semibold text-slate-500" for="lf-type">Type</label>
            <select
              id="lf-type"
              bind:value={itemType}
          ...
          <div>
            <label class="text-xs font-semibold text-slate-500" for="lf-category">Category</label>
            <select
              id="lf-category"
              bind:value={category}
              class="mt-1 h-10 w-full rounded-xl border border-cyan-100 bg-white px-3 text-sm text-slate-700 focus:border-cyan-300 focus:outline-hidden focus:ring-2 focus:ring-cyan-100"
        ...
        <div>
          <label class="text-xs font-semibold text-slate-500" for="lf-title">Title</label>
          <input
            id="lf-title"
            bind:value={title}
            autocomplete="off"
        ...
        <div>
          <label class="text-xs font-semibold text-slate-500" for="lf-description">Description</label>
          <textarea
            id="lf-description"
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="text-xs font-semibold text-slate-500" for="lf-date">Date</label>
            <input
              id="lf-date"
              type="date"
          ...
          <div>
            <label class="text-xs font-semibold text-slate-500" for="lf-location">Location</label>
            <input
              id="lf-location"
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="text-xs font-semibold text-slate-500" for="lf-contact">Contact Note (optional)</label>
            <input
              id="lf-contact"
          ...
          <div>
            <label class="text-xs font-semibold text-slate-500" for="lf-reward">Reward (optional)</label>
            <input
              id="lf-reward"
        ...
        <div>
          <label class="text-xs font-semibold text-slate-500" for="lost-found-files">Images (up to 5)</label>
          <input id="lost-found-files" type="file" accept="image/*" multiple onchange={handleFilesChange} class="hidden" />
